...
Phloem also requires that L<rsync> and L<ssh> (client and server) be
installed.
...
configuring host names, assigning IP addresses, installing L<rsync> and
L<ssh>, choosing or adding a user account for Phloem to use, generating
RSA keys for L<ssh>, creating directories for content etc.
...
Phloem relies on L<rsync> for its content transport duties. This greatly
...
Note that each subscriber will run an L<rsync> sub-process from time to
