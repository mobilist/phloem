...
Phloem also requires that L<rsync(1)> and L<ssh(1)> (client and server) be
installed.
...
configuring host names, assigning IP addresses, installing L<rsync(1)> and
L<ssh(1)>, choosing or adding a user account for Phloem to use, generating
RSA keys for L<ssh(1)>, creating directories for content etc.
...
Phloem relies on L<rsync(1)> for its content transport duties. This greatly
...
Note that each subscriber will run an L<rsync(1)> sub-process from time to
